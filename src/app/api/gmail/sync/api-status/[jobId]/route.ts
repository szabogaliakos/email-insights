...
export async function GET(request: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const job = GmailAPIScanner.getGmailApiJob(jobId);
      ...
      return NextResponse.json(
        {
          ...job,
          status: "failed",
          completeMessage: `Gmail API scan failed: ${job.error}`,
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ error: "Failed to get Gmail API job status" }, { status: 500 });
  }
}
